...
async function fixOrganizationCreation() {
  try {
    console.log('🔧 FIXING ORGANIZATION CREATION ISSUE');
    ...
    for (const orgData of organizationsToCreate) {
      ...
      try {
        // Create a temporary admin user first (hallAdmin doesn't require organization)
        ...
        const tempAdminUser = new User({
          ...
          role: 'hallAdmin', // Temporary role to bypass organization requirement
          roleLevel: 3,
          status: 'active',
          isVerified: true
        });
        
        const savedTempUser = await tempAdminUser.save();
        console.log(`   ✅ Temporary admin user created: ${savedTempUser.email}`);
        
        // Create the organization with the temp admin
        const organization = new Organization({
          ...
          adminId: savedTempUser._id, // Use temp admin
          contact: {
            email: orgData.adminEmail,
            phone: '+251911000000',
            address: {
        ...
        // Update the temp user to be orgAdmin with organization reference
        savedTempUser.role = 'orgAdmin';
        savedTempUser.roleLevel = 2;
        savedTempUser.organization = savedOrg._id;
        savedTempUser.permissions = [
          'manage_users',
          'manage_items',
          'view_reports',
          'manage_settings',
        ...
        console.log(`   ✅ Admin user updated to orgAdmin with organization reference`);
    ...
    allOrgs.forEach((org, index) => {
      ...
      console.log(`      Admin: ${org.adminId ? org.adminId.name + ' (' + org.adminId.email + ') - ' + org.adminId.role : 'No admin'}`);
    });
